...
func convertMinuteStats(stats []*model.EndpointMinuteStat) []*MinuteStatResponse {
	...
	for i, s := range stats {
		result[i] = &MinuteStatResponse{
			Timestamp: s.StatMinute, ActiveWorkers: s.ActiveWorkers, IdleWorkers: s.IdleWorkers,
			TasksSubmitted: s.TasksSubmitted, TasksCompleted: s.TasksCompleted,
			TasksFailed: s.TasksFailed, TasksTimeout: s.TasksTimeout,
			AvgQueueWaitMs: s.AvgQueueWaitMs, AvgExecutionMs: s.AvgExecutionMs, P95ExecutionMs: s.P95ExecutionMs,
		}
	}
	return result
}
...
func convertHourlyStats(stats []*model.EndpointHourlyStat) []*HourlyStatResponse {
	...
	for i, s := range stats {
		result[i] = &HourlyStatResponse{
			Timestamp: s.StatHour, ActiveWorkers: s.ActiveWorkers, IdleWorkers: s.IdleWorkers,
			TasksSubmitted: s.TasksSubmitted, TasksCompleted: s.TasksCompleted,
			TasksFailed: s.TasksFailed, TasksTimeout: s.TasksTimeout,
			AvgQueueWaitMs: s.AvgQueueWaitMs, AvgExecutionMs: s.AvgExecutionMs, P95ExecutionMs: s.P95ExecutionMs,
			ColdStarts: s.ColdStarts, AvgColdStartMs: s.AvgColdStartMs,
		}
	}
	return result
}
...
func convertDailyStats(stats []*model.EndpointDailyStat) []*DailyStatResponse {
	...
	for i, s := range stats {
		result[i] = &DailyStatResponse{
			Date: s.StatDate, ActiveWorkers: s.ActiveWorkers, IdleWorkers: s.IdleWorkers,
			TasksSubmitted: s.TasksSubmitted, TasksCompleted: s.TasksCompleted,
			TasksFailed: s.TasksFailed, TasksTimeout: s.TasksTimeout,
			AvgQueueWaitMs: s.AvgQueueWaitMs, AvgExecutionMs: s.AvgExecutionMs, P95ExecutionMs: s.P95ExecutionMs,
			ColdStarts: s.ColdStarts, AvgColdStartMs: s.AvgColdStartMs,
