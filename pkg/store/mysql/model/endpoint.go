...
type Endpoint struct {
	...
	ImagePrefix          string     `gorm:"column:image_prefix;type:varchar(500);not null;default:''" json:"image_prefix"`      // Image prefix for matching (e.g., "wavespeed/model-deploy:wan_i2v-default-")
	ImageDigest          string     `gorm:"column:image_digest;type:varchar(255);not null;default:''" json:"image_digest"`      // Current image digest from DockerHub
	ImageLastChecked     *time.Time `gorm:"column:image_last_checked;type:datetime(3)" json:"image_last_checked"`               // Last time image was checked for updates
	LatestImage          string     `gorm:"column:latest_image;type:varchar(500);not null;default:''" json:"latest_image"`      // Latest available image if update is available
	...
	MaxPendingTasks      int        `gorm:"column:max_pending_tasks;type:int;not null;default:1" json:"max_pending_tasks"` // Maximum allowed pending tasks before warning clients
